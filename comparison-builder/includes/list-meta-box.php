...
function wpc_render_list_meta_box($post)
{
    ...
    if ($enable_comparison === '')
        $enable_comparison = '1'; // Default to true
    ...
    if ($show_all_enabled === '')
        $show_all_enabled = '1'; // Default to enabled
    $initial_visible_count = get_post_meta($post->ID, '_wpc_list_initial_visible', true);
    if (empty($initial_visible_count))
        $initial_visible_count = '8'; // Default to 8
    ...
    if ($source_type === 'item' || $source_type === 'both')
        $query_post_types[] = 'comparison_item';
    if ($source_type === 'tool' || $source_type === 'both')
        $query_post_types[] = 'comparison_tool';
    if (empty($query_post_types))
        $query_post_types = array('comparison_item');
    ...
    <div class="wpc-list-config">
            ...
            jQuery(document).ready(function ($) {
                ...
                $('.wpc-sortable-list').sortable({
                    ...
                    update: function (event, ui) {
                        // Update row styling (alternate background colors)
                        $('.wpc-sortable-list tr').each(function (index) {
                            $(this).css('background-color', index % 2 === 0 ? '#fff' : '#f9f9f9');
                        });

                        // Move selected items to maintain visual priority
                        // (No automatic reordering - user explicitly controls order)
                    }
                });

                // Add placeholder styling
                $('<style>.sortable-placeholder { height: 50px; background: #e0f0ff !important; border: 2px dashed #0073aa; }</style>').appendTo('head');
        ...
        <div id="wpc-tab-general" class="wpc-tab-content active">
            ...
            <!-- Source Selection -->
            <div
                style="margin-bottom: 20px; padding: 15px; background: #f0f6fc; border: 1px solid #cce5ff; border-radius: 4px;">
                <label class="wpc-label" style="font-weight: bold; margin-bottom: 5px; display: block;">List Source
                    Type</label>
                <?php
                ...
                <p class="description">Select what type of content to include in this list. Click "Update" to refresh the
                    list below.</p>
            </div>
            ...
            <!-- Search Box -->
            <input type="text" id="wpc-list-search" placeholder="Search items..."
                style="width: 100%; margin-bottom: 10px; padding: 8px; font-size: 14px; border: 1px solid #ddd; border-radius: 4px;" />
            ...
            <div style="background: #fff; border: 1px solid #c3c4c7; max-height: 500px; overflow-y: auto;">
                <table class="widefat fixed striped" id="wpc-items-table">
                    <thead>
                        <tr>
                            <th style="width: 30px;"><span class="dashicons dashicons-menu" title="Drag to reorder"></span>
                            </th>
                            <th style="width: 60px;">
                            ...
                            <th style="width: 100px; text-align: center;">
                                <?php _e('Featured?', 'wp-comparison-builder'); ?></th>
                            <th style="width: 80px; text-align: center;"><?php _e('Schema?', 'wp-comparison-builder'); ?>
                            </th>
                    ...
                    <tbody class="wpc-sortable-list">
                        <?php if ($sorted_items): ?>
                            <?php foreach ($sorted_items as $item):
                                ...
                                ?>
                                <tr class="wpc-item-row <?php echo $is_selected ? 'selected-row' : ''; ?>"
                                    style="<?php echo $row_style; ?>" data-id="<?php echo $item->ID; ?>">
                                    ...
                                    <td>
                                        <input type="checkbox" name="item_ids[]" value="<?php echo esc_attr($item->ID); ?>"
                                            class="wpc-item-select" <?php checked($is_selected); ?> />
                                    </td>
                                    <td><strong><?php echo esc_html($item->post_title); ?></strong></td>
                                    <td style="text-align: center;">
                                        <input type="checkbox" name="featured_ids[]" value="<?php echo esc_attr($item->ID); ?>"
                                            <?php checked($is_featured); ?> />
                                    </td>
                                    <td style="text-align: center;">
                                        <?php $is_schema = in_array($item->ID, $schema_ids); ?>
                                        <input type="checkbox" name="schema_ids[]" value="<?php echo esc_attr($item->ID); ?>"
                                            <?php checked($is_schema); ?>
                                            title="Include this item in JSON-LD Schema (ItemList)" />
                                    </td>
                                    <td>
                                        <input type="text" name="badge_texts[<?php echo esc_attr($item->ID); ?>]"
                                            value="<?php echo esc_attr($badge_text); ?>" placeholder="e.g., Top Choice"
                                            style="width: 100%; border: 1px solid #ddd; border-radius: 3px;" />
                                    </td>
                                    <td>
                                        <input type="color" name="badge_colors[<?php echo esc_attr($item->ID); ?>]"
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="6"><?php _e('No items found.', 'wp-comparison-builder'); ?></td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            ...
            <div class="wpc-field-group wpc-custom-filters-group">
                ...
                <p class="description" style="margin-bottom: 10px;">Select terms to show as filter buttons above the list.
                </p>

                <!-- ITEM Filters -->
                <div id="wpc-item-filters"
                    style="display: <?php echo ($source_type === 'item' || $source_type === 'both') ? 'block' : 'none'; ?>;">
                        ...
                        <div style="flex: 1; border: 1px solid #ddd; padding: 10px; max-height: 150px; overflow-y: auto;">
                            ...
                            <?php if (!empty($all_cats) && !is_wp_error($all_cats)): ?>
                                <?php foreach ($all_cats as $cat): ?>
                                    <label style="display:block;"><input type="checkbox" name="wpc_list_filter_cats[]"
                                            value="<?php echo esc_attr($cat->term_id); ?>" <?php checked(in_array($cat->term_id, $filter_cats)); ?> /> <?php echo esc_html($cat->name); ?></label>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </div>
                        <div style="flex: 1; border: 1px solid #ddd; padding: 10px; max-height: 150px; overflow-y: auto;">
                            ...
                            <?php if (!empty($all_feats) && !is_wp_error($all_feats)): ?>
                                <?php foreach ($all_feats as $feat): ?>
                                    <label style="display:block;"><input type="checkbox" name="wpc_list_filter_feats[]"
                                            value="<?php echo esc_attr($feat->term_id); ?>" <?php checked(in_array($feat->term_id, $filter_feats)); ?> /> <?php echo esc_html($feat->name); ?></label>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </div>
                    </div>
                ...
                <!-- TOOL Filters -->
                <div id="wpc-tool-filters"
                    style="display: <?php echo ($tools_module_enabled && ($source_type === 'tool' || $source_type === 'both')) ? 'block' : 'none'; ?>;">
                        ...
                        <div style="flex: 1; border: 1px solid #ddd; padding: 10px; max-height: 150px; overflow-y: auto;">
                            ...
                            <?php if (!empty($all_tool_cats) && !is_wp_error($all_tool_cats)): ?>
                                <?php foreach ($all_tool_cats as $cat): ?>
                                    <label style="display:block;"><input type="checkbox" name="wpc_list_filter_tool_cats[]"
                                            value="<?php echo esc_attr($cat->term_id); ?>" <?php checked(in_array($cat->term_id, $filter_tool_cats)); ?> /> <?php echo esc_html($cat->name); ?></label>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </div>
                        <div style="flex: 1; border: 1px solid #ddd; padding: 10px; max-height: 150px; overflow-y: auto;">
                            ...
                            <?php if (!empty($all_tool_tags) && !is_wp_error($all_tool_tags)): ?>
                                <?php foreach ($all_tool_tags as $tag): ?>
                                    <label style="display:block;"><input type="checkbox" name="wpc_list_filter_tool_tags[]"
                                            value="<?php echo esc_attr($tag->term_id); ?>" <?php checked(in_array($tag->term_id, $filter_tool_tags)); ?> /> <?php echo esc_html($tag->name); ?></label>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </div>
                    </div>
            ...
            <div class="wpc-field-group">
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <textarea name="wpc_list_schema_desc" style="width: 100%; margin-top: 5px;" rows="3"
                        placeholder="Enter a unique description for the ItemList schema (search snippets). If empty, the list title will be used."><?php echo esc_textarea($schema_desc); ?></textarea>
                </div>
                ...
                <div class="wpc-flex-row">
                    <div class="wpc-flex-item">
                        ...
                        <input type="number" name="wpc_list_limit" value="<?php echo $limit ? esc_attr($limit) : ''; ?>"
                            style="width: 80px;" />
                        <span class="description">Leave empty for no limit.</span>
                    </div>
                    <div class="wpc-flex-item">
                        <label><strong>Initial Visible Count:</strong></label>
                        <input type="number" name="wpc_list_initial_visible"
                            value="<?php echo esc_attr($initial_visible_count); ?>" min="3" max="100"
                            style="width: 80px;" />
                    </div>
                    <div class="wpc-flex-item">
                        <label>
                ...
                <div style="background: #f0f0f1; border: 1px solid #ccd0d4; padding: 10px;">
                    ...
                    <code
                        style="font-size: 1.2em; margin-left: 10px; user-select: all;">[wpc_list id="<?php echo $post->ID; ?>"]</code>
                </div>
            </div>
        </div>
        ...
        <div id="wpc-tab-layout" class="wpc-tab-content">
            <div class="wpc-field-group">
                ...
                <div class="wpc-flex-row">
                    ...
                    <div class="wpc-flex-item">
                        ...
                        <select name="wpc_list_style" style="width: 100%;">
                            <option value="default" <?php selected($list_style, 'default'); ?>>Default (As Global)
                            </option>
                    ...
                    <div class="wpc-flex-item">
                        ...
                        <select name="wpc_list_pt_btn_pos_table" style="width: 100%;">
                            <option value="default" <?php selected($btn_pos_table, 'default'); ?>>Default (As Global)
                            </option>
                            <option value="after_price" <?php selected($btn_pos_table, 'after_price'); ?>>After Pricing
                            </option>
                            <option value="bottom" <?php selected($btn_pos_table, 'bottom'); ?>>Bottom (After Features)
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="wpc-flex-item">
                        ...
                        <select name="wpc_list_pt_btn_pos_popup" style="width: 100%;">
                            <option value="default" <?php selected($btn_pos_popup, 'default'); ?>>Default (As Global)
                            </option>
                            <option value="after_price" <?php selected($btn_pos_popup, 'after_price'); ?>>After Pricing
                            </option>
                            <option value="bottom" <?php selected($btn_pos_popup, 'bottom'); ?>>Bottom (After Features)
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="wpc-flex-item">
                        ...
                        <?php
                        $show_rating = get_post_meta($post->ID, '_wpc_list_show_rating', true);
                        if ($show_rating === '')
                            $show_rating = '1';
                        $show_price = get_post_meta($post->ID, '_wpc_list_show_price', true);
                        if ($show_price === '')
                            $show_price = '1';
                        ?>
                ...
                <div class="wpc-field-group" style="margin-top: 15px;">
                    ...
                    <p class="description">Control button visibility and features column for pricing tables and popups in
                        this list.</p>
                    <div class="wpc-flex-row">
                        <div class="wpc-flex-item">
                            <label class="wpc-field-label">"Select" Buttons</label>
                            <?php
                            $show_select_table = get_post_meta($post->ID, '_wpc_list_show_select_table', true);
                            if ($show_select_table === '')
                                $show_select_table = '1';
                            $show_select_popup = get_post_meta($post->ID, '_wpc_list_show_select_popup', true);
                            if ($show_select_popup === '')
                                $show_select_popup = '1';
                            ?>
                            <label style="display: block;"><input type="checkbox" name="wpc_list_show_select_table"
                                    value="1" <?php checked('1', $show_select_table); ?> /> Show in Table</label>
                            <label style="display: block;"><input type="checkbox" name="wpc_list_show_select_popup"
                                    value="1" <?php checked('1', $show_select_popup); ?> /> Show in Popup</label>
                        </div>
                        <div class="wpc-flex-item">
                            <label class="wpc-field-label">Footer Button</label>
                            <?php
                            $show_footer_table = get_post_meta($post->ID, '_wpc_list_show_footer_table', true);
                            if ($show_footer_table === '')
                                $show_footer_table = '1';
                            $show_footer_popup = get_post_meta($post->ID, '_wpc_list_show_footer_popup', true);
                            if ($show_footer_popup === '')
                                $show_footer_popup = '1';
                            ?>
                            <label style="display: block;"><input type="checkbox" name="wpc_list_show_footer_table"
                                    value="1" <?php checked('1', $show_footer_table); ?> /> Show in Table</label>
                            <label style="display: block;"><input type="checkbox" name="wpc_list_show_footer_popup"
                                    value="1" <?php checked('1', $show_footer_popup); ?> /> Show in Popup</label>
                        </div>
                        <div class="wpc-flex-item">
                            ...
                            ?>
                            <label style="display: block;"><input type="checkbox" name="wpc_list_hide_features" value="1"
                                    <?php checked('1', $hide_features); ?> /> Hide "Features" Column</label>
                        </div>
                    </div>
                    <button type="button" class="button button-small" style="margin-top: 10px;"
                        onclick="wpcResetPricingSettings()">🔄 Reset to Defaults</button>
                </div>
                <script>
                ...
                <div class="wpc-flex-row">
                    ...
                    <div class="wpc-flex-item">
                        ...
                        <select name="wpc_list_filter_layout" style="width: 100%;">
                            <option value="default" <?php selected($filter_layout, 'default'); ?>>Default (As Global)
                            </option>
                            <option value="top" <?php selected($filter_layout, 'top'); ?>>Horizontal (Top)</option>
                            <option value="sidebar" <?php selected($filter_layout, 'sidebar'); ?>>Vertical (Sidebar)
                            </option>
                        </select>
                    </div>
                    <!-- Show Filters -->
                ...
                <div class="wpc-flex-row">
                    ...
                    <div class="wpc-flex-item">
                        ...
                        <select name="wpc_list_search_type" style="width: 100%;">
                            ...
                            <option value="combobox" <?php selected($search_type, 'combobox'); ?>>Advanced Combobox
                            </option>
                        </select>
                    </div>
                </div>
        ...
        <div id="wpc-tab-text" class="wpc-tab-content">
            <div class="wpc-field-group">
                ...
                <div class="wpc-flex-row">
                    ...
                    <div class="wpc-flex-item">
                        ...
                        <input type="text" name="wpc_list_txt_search_ph"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_search_ph', true) ?: 'Search & Select...'); ?>"
                            style="width: 100%;" />
                    </div>
                </div>
                <div class="wpc-flex-row">
                    ...
                    <div class="wpc-flex-item">
                        ...
                        <input type="text" name="wpc_list_txt_active_filt"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_active_filt', true) ?: 'Active filters:'); ?>"
                            style="width: 100%;" />
                    </div>
                    <!-- Clear All -->
                    <div class="wpc-flex-item">
                        <label class="wpc-field-label">"Clear all" Label</label>
                        <input type="text" name="wpc_list_txt_clear_all"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_clear_all', true) ?: 'Clear all'); ?>"
                            style="width: 100%;" />
                    </div>
                </div>
                <div class="wpc-flex-row">
                    ...
                    <div class="wpc-flex-item">
                        ...
                        <input type="text" name="wpc_list_txt_no_results"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_no_results', true) ?: 'No items match your filters.'); ?>"
                            style="width: 100%;" />
                    </div>
                    <!-- Selected: -->
                    <div class="wpc-flex-item">
                        <label class="wpc-field-label">"Selected:" Label</label>
                        <input type="text" name="wpc_list_txt_selected"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_selected', true) ?: 'Selected:'); ?>"
                            style="width: 100%;" />
                    </div>
                </div>
            </div>
            ...
            <div class="wpc-field-group">
                ...
                <div class="wpc-flex-row">
                    <div class="wpc-flex-item">
                        ...
                        <input type="text" name="wpc_list_txt_sort_def"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_sort_def', true) ?: 'Sort: Default'); ?>"
                            style="width: 100%;" />
                    </div>
                    <div class="wpc-flex-item">
                        <label class="wpc-field-label">Name (A-Z)</label>
                        <input type="text" name="wpc_list_txt_sort_asc"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_sort_asc', true) ?: 'Name (A-Z)'); ?>"
                            style="width: 100%;" />
                    </div>
                    <div class="wpc-flex-item">
                        <label class="wpc-field-label">Name (Z-A)</label>
                        <input type="text" name="wpc_list_txt_sort_desc"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_sort_desc', true) ?: 'Name (Z-A)'); ?>"
                            style="width: 100%;" />
                    </div>
                </div>
                <div class="wpc-flex-row">
                    <div class="wpc-flex-item">
                        ...
                        <input type="text" name="wpc_list_txt_sort_rating"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_sort_rating', true) ?: 'Highest Rated'); ?>"
                            style="width: 100%;" />
                    </div>
                    <div class="wpc-flex-item">
                        <label class="wpc-field-label">Lowest Price</label>
                        <input type="text" name="wpc_list_txt_sort_price"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_sort_price', true) ?: 'Lowest Price'); ?>"
                            style="width: 100%;" />
                    </div>
                </div>
            </div>
            ...
            <div class="wpc-field-group">
                ...
                <div class="wpc-flex-row">
                    <div class="wpc-flex-item">
                        ...
                        <input type="text" name="wpc_list_txt_get_coupon"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_get_coupon', true) ?: 'Get Coupon:'); ?>"
                            style="width: 100%;" />
                    </div>
                    <div class="wpc-flex-item">
                        <label class="wpc-field-label">"Featured" Badge Text</label>
                        <input type="text" name="wpc_list_txt_featured"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_featured', true) ?: 'Featured'); ?>"
                            style="width: 100%;" />
                ...
                <div class="wpc-flex-row">
                    <div class="wpc-flex-item">
                        ...
                        <input type="text" name="wpc_list_txt_feat_prod"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_feat_prod', true) ?: 'Products'); ?>"
                            style="width: 100%;" />
                    </div>
                    <div class="wpc-flex-item">
                        <label class="wpc-field-label">"Trans. Fees" Suffix</label>
                        <input type="text" name="wpc_list_txt_feat_fees"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_feat_fees', true) ?: 'Trans. Fees'); ?>"
                            style="width: 100%;" />
                    </div>
                    <div class="wpc-flex-item">
                        <label class="wpc-field-label">"Support" Suffix</label>
                        <input type="text" name="wpc_list_txt_feat_supp"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_feat_supp', true) ?: 'Support'); ?>"
                            style="width: 100%;" />
                    </div>
                </div>
                <div class="wpc-flex-row">
                    <div class="wpc-flex-item">
                        ...
                        <input type="text" name="wpc_list_txt_feat_channels"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_feat_channels', true) ?: 'Sales Channels'); ?>"
                            style="width: 100%;" />
                    </div>
                    <div class="wpc-flex-item">
                        <label class="wpc-field-label">"Free SSL" Label</label>
                        <input type="text" name="wpc_list_txt_feat_ssl"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_feat_ssl', true) ?: 'Free SSL'); ?>"
                            style="width: 100%;" />
                    </div>
                </div>
                <div class="wpc-flex-row">
                    <div class="wpc-flex-item">
                        ...
                        <input type="text" name="wpc_list_cat_label" class="wpc-input-cat-label"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_cat_label', true) ?: 'Filter by Category'); ?>"
                            style="width: 100%;" />
                    </div>
                    <div class="wpc-flex-item">
                        <label class="wpc-field-label">Features Label</label>
                        <input type="text" name="wpc_list_feat_label" class="wpc-input-feat-label"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_feat_label', true) ?: 'Features'); ?>"
                            style="width: 100%;" />
                    </div>
                </div>
            </div>
            ...
            <div class="wpc-field-group">
                ...
                <div class="wpc-flex-row">
                    ...
                    <div class="wpc-flex-item">
                        ...
                        <input type="text" name="wpc_list_txt_compare"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_compare', true) ?: 'Select to Compare'); ?>"
                            style="width: 100%;" />
                    </div>
                    <!-- Copied! Text -->
                    <div class="wpc-flex-item">
                        <label class="wpc-field-label">"Copied!" Text</label>
                        <input type="text" name="wpc_list_txt_copied"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_copied', true) ?: 'Copied!'); ?>"
                            style="width: 100%;" />
                    </div>
                </div>
                <div class="wpc-flex-row">
                    ...
                    <div class="wpc-flex-item">
                        ...
                        <input type="text" name="wpc_list_txt_view"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_view', true) ?: 'View Details'); ?>"
                            style="width: 100%;" />
                    </div>
                    <!-- Visit Site Text -->
                    <div class="wpc-flex-item">
                        <label class="wpc-field-label">"Visit Site" Text</label>
                        <input type="text" name="wpc_list_txt_visit"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_visit', true) ?: 'Visit Site'); ?>"
                            style="width: 100%;" />
                    </div>
                </div>
                <div class="wpc-flex-row">
                    ...
                    <div class="wpc-flex-item">
                        ...
                        <input type="text" name="wpc_list_txt_compare_btn"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_compare_btn', true) ?: 'Compare (%s) Items'); ?>"
                            style="width: 100%;" />
                    </div>
                </div>
                <div class="wpc-flex-row">
                    ...
                    <div class="wpc-flex-item">
                        ...
                        <input type="text" name="wpc_list_txt_compare_now"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_compare_now', true) ?: 'Compare Now'); ?>"
                            style="width: 100%;" />
                    </div>
                    <!-- Visit Platform Text -->
                    <div class="wpc-flex-item">
                        <label class="wpc-field-label">"Visit Platform" Text</label>
                        <input type="text" name="wpc_list_txt_visit_plat"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_visit_plat', true) ?: 'Visit %s'); ?>"
                            style="width: 100%;" />
                    </div>
                    <!-- Comparison Header -->
                    <div class="wpc-flex-item">
                        <label class="wpc-field-label">"Detailed Comparison" Header</label>
                        <input type="text" name="wpc_list_txt_comp_header"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_comp_header', true) ?: 'Detailed Comparison'); ?>"
                            style="width: 100%;" />
                    </div>
                </div>
                ...
                <div class="wpc-field-group">
                    ...
                    <div class="wpc-flex-row">
                        <div class="wpc-flex-item">
                            ...
                            <input type="text" name="wpc_list_txt_feat_header"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_feat_header', true) ?: 'Feature'); ?>"
                                style="width: 100%;" />
                        </div>
                        <div class="wpc-flex-item">
                            <label class="wpc-field-label">"Price" Row Header</label>
                            <input type="text" name="wpc_list_txt_price"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_price', true) ?: 'Price'); ?>"
                                style="width: 100%;" />
                        </div>
                    </div>
                    <div class="wpc-flex-row">
                        <div class="wpc-flex-item">
                            ...
                            <input type="text" name="wpc_list_txt_pros"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_pros', true) ?: 'Pros'); ?>"
                                style="width: 100%;" />
                        </div>
                        <div class="wpc-flex-item">
                            <label class="wpc-field-label">"Cons" Row Header</label>
                            <input type="text" name="wpc_list_txt_cons"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_cons', true) ?: 'Cons'); ?>"
                                style="width: 100%;" />
                        </div>
                    </div>
                    <div class="wpc-flex-row">
                        <div class="wpc-flex-item">
                            ...
                            <input type="text" name="wpc_list_txt_rating"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_rating', true) ?: 'Rating'); ?>"
                                style="width: 100%;" />
                        </div>
                        <div class="wpc-flex-item">
                            <label class="wpc-field-label">Month Suffix (/mo)</label>
                            <input type="text" name="wpc_list_txt_mo_suffix"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_mo_suffix', true) ?: '/mo'); ?>"
                                style="width: 100%;" />
                        </div>
                    </div>
                </div>
                ...
                <div class="wpc-field-group">
                    ...
                    <div class="wpc-flex-row">
                        <div class="wpc-flex-item">
                            ...
                            <input type="text" name="wpc_list_txt_select_plan"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_select_plan', true) ?: 'Select'); ?>"
                                style="width: 100%;" />
                        </div>
                        <div class="wpc-flex-item">
                            <label class="wpc-field-label">"Plan" Column Header</label>
                            <input type="text" name="wpc_list_txt_pt_plan"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_pt_plan', true) ?: 'Plan'); ?>"
                                style="width: 100%;" />
                        </div>
                    </div>
                    <div class="wpc-flex-row">
                        <div class="wpc-flex-item">
                            ...
                            <input type="text" name="wpc_list_txt_pt_price"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_pt_price', true) ?: 'Price'); ?>"
                                style="width: 100%;" />
                        </div>
                        <div class="wpc-flex-item">
                            <label class="wpc-field-label">"Features" Column Header (Table)</label>
                            <input type="text" name="wpc_list_txt_pt_features"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_pt_features', true) ?: 'Features'); ?>"
                                style="width: 100%;" />
                        </div>
                    </div>
                </div>
                ...
                <div class="wpc-field-group">
                    ...
                    <div class="wpc-flex-row">
                        <div class="wpc-flex-item">
                            ...
                            <input type="text" name="wpc_list_txt_no_compare"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_no_compare', true) ?: 'Select up to 4 items to compare'); ?>"
                                style="width: 100%;" />
                        </div>
                        <div class="wpc-flex-item">
                            <label class="wpc-field-label">"Remove" Text</label>
                            <input type="text" name="wpc_list_txt_remove"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_remove', true) ?: 'Remove'); ?>"
                                style="width: 100%;" />
                        </div>
                    </div>
                    <div class="wpc-flex-row">
                        <div class="wpc-flex-item">
                            ...
                            <input type="text" name="wpc_list_txt_logo"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_logo', true) ?: 'Logo'); ?>"
                                style="width: 100%;" />
                        </div>
                        <div class="wpc-flex-item">
                            <label class="wpc-field-label">"Analysis" Suffix</label>
                            <input type="text" name="wpc_list_txt_analysis"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_analysis', true) ?: '(Based on our analysis)'); ?>"
                                style="width: 100%;" />
                        </div>
                    </div>
                    <div class="wpc-flex-row">
                        <div class="wpc-flex-item">
                            ...
                            <input type="text" name="wpc_list_txt_start_price"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_start_price', true) ?: 'Starting Price'); ?>"
                                style="width: 100%;" />
                        </div>
                        <div class="wpc-flex-item">
                            <label class="wpc-field-label">"Dashboard Preview" Label</label>
                            <input type="text" name="wpc_list_txt_dash_prev"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_dash_prev', true) ?: 'Dashboard Preview'); ?>"
                                style="width: 100%;" />
                        </div>
                    </div>
                </div>
                ...
                <div class="wpc-field-group">
                    ...
                    <div class="wpc-flex-row">
                        <div class="wpc-flex-item">
                            ...
                            <input type="text" name="wpc_list_txt_reset_filt"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_reset_filt', true) ?: 'Reset Filters'); ?>"
                                style="width: 100%;" />
                        </div>
                        <div class="wpc-flex-item">
                            <label class="wpc-field-label">"Select %s" Format</label>
                            <input type="text" name="wpc_list_txt_select_fmt"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_select_fmt', true) ?: 'Select %s'); ?>"
                                style="width: 100%;" />
                        </div>
                    </div>
                    <div class="wpc-flex-row">
                        <div class="wpc-flex-item">
                            ...
                            <input type="text" name="wpc_list_txt_clear"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_clear', true) ?: 'Clear'); ?>"
                                style="width: 100%;" />
                        </div>
                        <div class="wpc-flex-item">
                            <label class="wpc-field-label">"Select provider..." Placeholder</label>
                            <input type="text" name="wpc_list_txt_sel_prov"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_sel_prov', true) ?: 'Select provider...'); ?>"
                                style="width: 100%;" />
                        </div>
                    </div>
                    <div class="wpc-flex-row">
                        <div class="wpc-flex-item">
                            ...
                            <input type="text" name="wpc_list_txt_no_item"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_no_item', true) ?: 'No item found.'); ?>"
                                style="width: 100%;" />
                        </div>
                        <div class="wpc-flex-item">
                            <label class="wpc-field-label">"more" (e.g. +2 more)</label>
                            <input type="text" name="wpc_list_txt_more"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_more', true) ?: 'more'); ?>"
                                style="width: 100%;" />
                        </div>
                    </div>
                    <div class="wpc-flex-row">
                        <div class="wpc-flex-item">
                            ...
                            <input type="text" name="wpc_list_txt_show_all"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_show_all', true) ?: 'Show All Items'); ?>"
                                style="width: 100%;" />
                        </div>
                        <div class="wpc-flex-item">
                            <label class="wpc-field-label">"Click to reveal X more" Text</label>
                            <input type="text" name="wpc_list_txt_reveal_more"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_reveal_more', true) ?: 'Click to reveal'); ?>"
                                style="width: 100%;" />
                        </div>
                        <div class="wpc-flex-item">
                            <label class="wpc-field-label">"No Logo" Fallback Text</label>
                            <input type="text" name="wpc_list_txt_no_logo"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_txt_no_logo', true) ?: 'No Logo'); ?>"
                                style="width: 100%;" />
                        </div>
                    </div>
                </div>
            ...
            <div class="wpc-field-group">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                    <div>
                        <label>Primary Color</label> <input type="color" name="wpc_list_primary_color"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_primary_color', true) ?: '#6366f1'); ?>" />
                    ...
                    <div>
                        <label>Accent Color</label> <input type="color" name="wpc_list_accent_color"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_accent_color', true) ?: '#0d9488'); ?>" />
                    ...
                    <div>
                        <label>Button Hover</label> <input type="color" name="wpc_list_hover_color"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_hover_color', true) ?: '#059669'); ?>" />
                    ...
                    <div>
                        <label>Header BG</label> <input type="color" name="wpc_list_pt_header_bg"
                            value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_pt_header_bg', true) ?: '#f8fafc'); ?>" />
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 15px;">
                    <div>
                        ...
                        <div style="display:flex; align-items:center; gap:5px;">
                            <input type="color" name="wpc_list_color_pros_bg"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_color_pros_bg', true) ?: '#f0fdf4'); ?>" />
                    ...
                    <div>
                        ...
                        <div style="display:flex; align-items:center; gap:5px;">
                            <input type="color" name="wpc_list_color_pros_text"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_color_pros_text', true) ?: '#166534'); ?>" />
                    ...
                    <div>
                        ...
                        <div style="display:flex; align-items:center; gap:5px;">
                            <input type="color" name="wpc_list_color_cons_bg"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_color_cons_bg', true) ?: '#fef2f2'); ?>" />
                    ...
                    <div>
                        ...
                        <div style="display:flex; align-items:center; gap:5px;">
                            <input type="color" name="wpc_list_color_cons_text"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_color_cons_text', true) ?: '#991b1b'); ?>" />
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap: 15px;">
                    <div>
                        ...
                        <div style="display:flex; align-items:center; gap:5px;">
                            <input type="color" name="wpc_list_color_coupon_bg"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_color_coupon_bg', true) ?: '#fef3c7'); ?>" />
                    ...
                    <div>
                        ...
                        <div style="display:flex; align-items:center; gap:5px;">
                            <input type="color" name="wpc_list_color_coupon_text"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_color_coupon_text', true) ?: '#92400e'); ?>" />
                    ...
                    <div>
                        ...
                        <div style="display:flex; align-items:center; gap:5px;">
                            <input type="color" name="wpc_list_color_coupon_hover"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_color_coupon_hover', true) ?: '#fde68a'); ?>" />
                    ...
                    <div>
                        ...
                        <div style="display:flex; align-items:center; gap:5px;">
                            <input type="color" name="wpc_list_color_copied"
                                value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_color_copied', true) ?: '#10b981'); ?>" />
        ...
        <div id="wpc-tab-comparison" class="wpc-tab-content">
            <div class="wpc-field-group">
                ...
                <div
                    style="background: #fdfdfd; padding: 20px; border: 1px solid #e5e7eb; border-left: 4px solid #6366f1; border-radius: 4px; margin-bottom: 25px;">
                    ...
                    <p class="description" style="margin-left: 25px;">
                        <strong>ON</strong>: Users can select cards to compare. Selection circles and Footer bar can be
                        enabled.<br>
                        <strong>OFF</strong>: All comparison features are disabled. Selection is blocked. Cards only allow
                        "Details".
                    </p>
                </div>

                <div class="wpc-comparison-options"
                    style="margin-bottom: 20px; padding-left: 25px; <?php echo ($enable_comparison === '1') ? '' : 'opacity:0.6; pointer-events:none;'; ?>">
            ...
            <div class="wpc-field-group">
                ...
                <div class="wpc-flex-row">
                    <div class="wpc-flex-item">
                        ...
                        <input type="text" name="wpc_list_button_text"
                            value="<?php echo esc_attr($custom_button_text); ?>" placeholder="e.g. Visit Site"
                            style="width:100%;" />
                    </div>
                    <div class="wpc-flex-item">
        ...
        <div id="wpc-tab-features" class="wpc-tab-content">
            <div class="wpc-field-group">
                <h3 style="margin-top:0;"><?php _e('Comparison Table Features & Hierarchy', 'wp-comparison-builder'); ?>
                </h3>
                <p class="description">
                    <?php _e('Customize the features and their order specifically for this list.', 'wp-comparison-builder'); ?>
                </p>
                ...
                <div
                    ...
                    <p class="description" style="margin-left: 20px; margin-top: 5px;">
                        Check this box to manually select which columns appear in this specific table. If unchecked, the
                        Global Settings features will be used.
                    </p>
                    ...
                    <div style="margin-top: 15px; margin-left: 20px; padding-top: 15px; border-top: 1px dashed #ddd;">
                        ...
                        <div style="display:flex; gap: 20px;">
                            <div>
                                ...
                                <div style="display:flex; align-items:center; gap:5px;">
                                    <input type="color" name="wpc_list_color_tick"
                                        value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_color_tick', true) ?: '#10b981'); ?>"
                            ...
                            <div>
                                ...
                                <div style="display:flex; align-items:center; gap:5px;">
                                    <input type="color" name="wpc_list_color_cross"
                                        value="<?php echo esc_attr(get_post_meta($post->ID, '_wpc_list_color_cross', true) ?: '#ef4444'); ?>"
                                        style="height:30px; cursor:pointer;" />
                ...
                <div id="wpc-list-features-ui" style="<?php echo $override_global ? '' : 'display:none;'; ?>">
                    ...
                    <div class="wpc-dual-listbox-wrapper">
                        ...
                        <style>
                            ...
                            .wpc-sortable-list li:hover {
                                background: #f0f6fc;
                            }

                            .wpc-sortable-list li.ui-sortable-helper {
                        ...
                        <div class="wpc-list-col">
                            <div class="wpc-list-header">
                                ...
                                <button type="button" class="button button-small"
                                    id="wpc-list-add-all"><?php _e('Add All', 'wp-comparison-builder'); ?></button>
                            </div>
                            <div class="wpc-list-search">
                                <input type="text" id="wpc-list-search-available"
                                    placeholder="<?php _e('Search tags...', 'wp-comparison-builder'); ?>">
                            </div>
                                ...
                                <?php foreach ($available_keys as $key):
                                    ...
                                    <li data-key="<?php echo esc_attr($key); ?>">
                                        <span class="wpc-item-label">
                                            ...
                                            <span
                                                class="wpc-badge <?php echo esc_attr($item['type']); ?>"><?php echo $item['type'] === 'builtin' ? 'Built-in' : 'Tag'; ?></span>
                                        </span>
                        ...
                        <div class="wpc-list-col">
                            <div class="wpc-list-header">
                                ...
                                <button type="button" class="button button-small"
                                    id="wpc-list-remove-all"><?php _e('Remove All', 'wp-comparison-builder'); ?></button>
                            </div>
                            ...
                            <div class="wpc-empty-msg"><?php _e('Drag items here to active', 'wp-comparison-builder'); ?>
                            </div>
                        </div>
                    </div>
                </div>
