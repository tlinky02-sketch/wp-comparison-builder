        ...
        while ($query->have_posts()) {
            ...
            if ($post_type === 'comparison_tool') {
                ...
                if (!$categories || is_wp_error($categories))
                    $categories = wp_get_post_terms($id, 'tool_category');

                $features = get_the_terms($id, 'tool_tag');
                if (!$features || is_wp_error($features))
                    $features = wp_get_post_terms($id, 'tool_tag');
            } else {
            ...
            if ($categories && !is_wp_error($categories)) {
                foreach ($categories as $c) {
                    ...
                    if (!in_array($c->name, $all_categories))
            ...
            if ($features && !is_wp_error($features)) {
                foreach ($features as $f) {
                    $name = $f->name;
                    $feature_names[] = $name;
                    if (!in_array($name, $all_feature_terms))
                        $all_feature_terms[] = $name;

                    // Simple mapping for tools as well
                    $name_lower = strtolower($name);
                    if (strpos($name_lower, 'ssl') !== false)
                        $feature_map['ssl'] = true;
                    if (strpos($name_lower, 'support') !== false)
                        $feature_map['support'] = $name;

                    // Products
                    if (strpos($name_lower, 'product') !== false) {
                        $feature_map['products'] = trim(str_ireplace(array('products', 'unlimited'), '', $name));
                        if (empty($feature_map['products']))
                            $feature_map['products'] = 'Unlimited';
                    }
            ...
            if ($post_type === 'comparison_tool') {
                ...
                if (isset($tools_db_data[$id])) {
                    ...
                    $period = '';
                    $rating = floatval(get_post_meta($id, '_wpc_tool_rating', true) ?: 4.5);
                ...
                $rating = floatval($get_val('rating', '_wpc_rating', '_ecommerce_rating', 4.0));
            ...
            $items[] = array(
                ...
                'variants' => (function () use ($id, $post_type) {
                    if ($post_type !== 'comparison_item')
                        return null;
                    if (get_option('wpc_enable_variants_module') !== '1')
                        return null;
                    if (get_post_meta($id, '_wpc_variants_enabled', true) !== '1')
                        return null;
