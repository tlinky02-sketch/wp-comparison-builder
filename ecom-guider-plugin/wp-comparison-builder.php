...
function wpc_register_scripts()
{
    ...
    if (!empty($google_fonts)) {
        $font_url = 'https://fonts.googleapis.com/css2?' . implode('&', array_map(function ($f) {
            return 'family=' . $f;
    ...
    if (!empty($font_size_h1))
        $custom_css .= " --wpc-font-size-h1: {$font_size_h1}px; ";
    if (!empty($font_size_h2))
        $custom_css .= " --wpc-font-size-h2: {$font_size_h2}px; ";
    if (!empty($font_size_h3))
        $custom_css .= " --wpc-font-size-h3: {$font_size_h3}px; ";
    if (!empty($font_size_h4))
        $custom_css .= " --wpc-font-size-h4: {$font_size_h4}px; ";
    if (!empty($font_size_h5))
        $custom_css .= " --wpc-font-size-h5: {$font_size_h5}px; ";
    if (!empty($font_size_h6))
        $custom_css .= " --wpc-font-size-h6: {$font_size_h6}px; ";
    if (!empty($font_size_btn))
        $custom_css .= " --wpc-font-size-btn: {$font_size_btn}px; ";
    if (!empty($font_size_price))
        $custom_css .= " --wpc-font-size-price: {$font_size_price}px; ";
    if (!empty($font_size_code))
        $custom_css .= " --wpc-font-size-code: {$font_size_code}px; ";
    ...
    $custom_css .= "
        :root {
            ...
            --wpc-star-color: " . esc_attr(get_option('wpc_star_rating_color', '#fbbf24') ?: '#fbbf24') . ";
            --wpc-btn-text: " . esc_attr(get_option('wpc_button_text_color', '#ffffff') ?: '#ffffff') . ";
            --pt-btn-text: " . esc_attr(get_option('wpc_button_text_color', '#ffffff') ?: '#ffffff') . ";
        }
    ";
    ...
    if (!empty($category_slug)) {
        $items = array_filter($items, function ($item) use ($category_slug) {
            ...
            foreach ($item['category'] as $cat) {
                if (md5(strtolower($cat)) === md5(strtolower($category_slug)) || strtolower($cat) === strtolower($category_slug))
                    return true;
                            ...
                            <div class="flex-1">
                                ...
                                <div class="<?php echo esc_attr($grid_class); ?>">
                                    <?php foreach ($items as $item):
                                        ...
                                        ?>
                                            <div class="group relative rounded-2xl border bg-card text-card-foreground shadow-sm transition-all hover:shadow-md"
                                                style="<?php echo esc_attr($style_attr); ?>">
                                                <?php if (!empty($featured_badge_text)): ?>
                                                        <div
                                                            class="absolute -top-3 left-1/2 -translate-x-1/2 z-10 px-3 py-1 bg-primary text-primary-foreground text-xs font-bold uppercase tracking-wider rounded-full shadow-sm">
                                                ...
                                                <div class="p-6">
                                                    <div class="flex items-start justify-between mb-4">
                                                        <div class="flex items-center gap-3">
                                                            ...
                                                            <?php endif; ?>
                                                            <div>
                                                                <h3 class="font-bold text-lg leading-none mb-1"><?php echo esc_html($name); ?>
                                                                </h3>
                                                                <div class="flex items-center gap-1">
                                                                    ...
                                                                    <span
                                                                        class="text-xs text-muted-foreground font-medium">(<?php echo esc_html($rating); ?>)</span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="mb-6 p-3 bg-muted/30 rounded-xl text-center">
                                                        <div class="flex items-baseline justify-center gap-1">
                                                            ...
                                                            <?php if ($period): ?><span
                                                                        class="text-sm font-medium text-muted-foreground"><?php echo esc_html($period); ?></span><?php endif; ?>
                                                        </div>
                                                    </div>
                ...
                <?php else: ?>
                        ...
                        <div class="w-full flex flex-col lg:grid lg:grid-cols-4 lg:gap-8">
                            ...
                            <div class="w-full flex flex-col lg:grid lg:grid-cols-4 lg:gap-8">
                                <!-- Static Sidebar Skeleton -->
                                <div
                                    class="lg:col-span-1 border border-border rounded-xl p-6 bg-card mb-8 lg:mb-0 h-fit lg:sticky lg:top-24">
                                    <div class="flex items-center gap-2 mb-2 pb-2 border-b border-border">
                                        <?php echo $icon_filter; ?>
                                        <span
                                            class="font-display font-bold text-lg text-foreground"><?php _e('Filters', 'wp-comparison-builder'); ?></span>
                                    </div>
                                                    ...
                                                    <div class="p-6">
                                                        <div class="flex items-start justify-between mb-4">
                                                            <div class="flex items-center gap-3">
                                                                <?php if ($logo): ?>
                                                                        <div
                                                                            class="w-12 h-12 rounded-lg bg-muted/10 p-1 flex items-center justify-center">
                                                                            <img src="<?php echo esc_url($logo); ?>"
                                                                                class="w-full h-full object-contain" />
                                                                        </div>
                                                                <?php endif; ?>
                                                                <div>
                                                                    <h3 class="font-bold text-lg"><?php echo esc_html($name); ?></h3>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="mb-6 p-3 bg-muted/30 rounded-xl text-center">
...
function wpc_list_shortcode($atts)
{
    ...
    if (empty($badge_texts_raw))
        $badge_texts_raw = get_post_meta($post_id, '_hg_list_badge_texts', true);

    // Badge Colors
    $badge_colors_raw = get_post_meta($post_id, '_wpc_list_badge_colors', true);
    if (empty($badge_colors_raw))
        $badge_colors_raw = get_post_meta($post_id, '_hg_list_badge_colors', true);
    ...
    $show_filters = true;
    if ($show_filters_opt === 'show')
        $show_filters = true;
    elseif ($show_filters_opt === 'hide')
        $show_filters = false;
    else {
    ...
    $show_search = true;
    if ($show_search_opt === 'show')
        $show_search = true;
    elseif ($show_search_opt === 'hide')
        $show_search = false;
    else {
    ...
    $show_plans = true;
    if ($show_plans_override === '1')
        $show_plans = true;
    elseif ($show_plans_override === '0')
        $show_plans = false;
    else {
    ...
    if (!empty($specific_ids)) {
        ...
        usort($items, function ($a, $b) use ($specific_ids, $featured_ids_array) {
            ...
            if ($is_a_featured && !$is_b_featured)
                return -1;
            if (!$is_a_featured && $is_b_featured)
                return 1;
    ...
    foreach ($text_fields as $key => $source) {
        ...
        if (empty($val)) {
            ...
            if (empty($val))
                $val = $source['default']; // extra safety
        }
        $labels[$key] = $val;
    }
    ...
    if ($use_border) {
        ...
        if ($c)
            $css_rules .= "#{$unique_id} .bg-card { border-color: {$c} !important; } ";
    }
                        ...
                        ?>
                            ...
                            <?php if (is_array($billing_cycles) && count($billing_cycles) > 1): ?>
                                    <div style="display:flex; justify-content:center; margin-bottom:1rem; width:100%;">
                                        <div
                                            style="display:inline-flex; border-radius:0.5rem; border:1px solid hsl(var(--border)); background:hsl(var(--muted)); padding:0.25rem; gap:0.25rem;">
                                            <?php foreach ($billing_cycles as $cycle):
                                                $is_active = ($cycle['slug'] === $default_cycle);
                                                ?>
                                                    <span
                                                        style="padding:0.5rem 1rem; border-radius:0.375rem; font-size:var(--wpc-font-size-base); font-weight:500; cursor:pointer; <?php echo $is_active ? 'background:hsl(var(--primary)); color:hsl(var(--primary-foreground)); box-shadow:0 1px 2px rgba(0,0,0,0.05);' : 'color:hsl(var(--muted-foreground)); background:transparent;'; ?>">
                            ...
                            <div
                                style="width:100%; border:1px solid hsl(var(--border)); border-radius:0.75rem; background:hsl(var(--card)); overflow-x:auto;">
                                <table style="width:100%; min-width:max-content; border-collapse:collapse; text-align:center;">
                                    <thead>
                                        <tr>
                                            <?php foreach ($plans as $idx => $plan): ?>
                                                    <th
                                                        style="padding:1rem; border-bottom:1px solid hsl(var(--border)); background-color:var(--pt-header-bg); color:var(--pt-header-text); min-width:260px; vertical-align:top; border-right:<?php echo ($idx < count($plans) - 1) ? '1px solid hsl(var(--border))' : 'none'; ?>;">
                                                        <div style="font-size:var(--wpc-font-size-h2); font-weight:700;">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <?php foreach ($plans as $idx => $plan):
                                                ...
                                                ?>
                                                    <td
                                                        style="padding:1.5rem 1rem; vertical-align:top; border-right:<?php echo ($idx < count($plans) - 1) ? '1px solid var(--pt-border)' : 'none'; ?>;">
                                                        <div
                                                            style="display:flex; flex-wrap:wrap; align-items:baseline; justify-content:center; gap:0.25rem;">
                                                            <span
                                                                style="font-size:var(--wpc-font-size-price, var(--wpc-font-size-h2)); font-weight:700; color:hsl(var(--primary));"><?php echo esc_html($ssr_price); ?></span>
                                                            <?php if (!empty($ssr_period)): ?>
                                                                    <span
                                                                        style="color:hsl(var(--muted-foreground)); font-size:calc(var(--wpc-font-size-price) * 0.5); line-height:1.2;"><?php echo esc_html($ssr_period); ?></span>
                                                            <?php endif; ?>
                                                        </div>
                                                    </td>
                                            <?php endforeach; ?>
                                        ...
                                        <tr style="border-top:1px solid var(--pt-border);">
                                            <?php foreach ($plans as $idx => $plan):
                                                ...
                                                ?>
                                                    <td
                                                        style="padding:1rem; vertical-align:top; text-align:left; border-right:<?php echo ($idx < count($plans) - 1) ? '1px solid var(--pt-border)' : 'none'; ?>;">
                                                        <?php if (!empty($features)): ?>
                                                                <ul style="list-style:none; padding:0; margin:0; color:hsl(var(--muted-foreground));">
                                                                    <?php foreach ($features as $feature):
                                                                        ...
                                                                        ?>
                                                                            <li
                                                                                style="margin-bottom:0.5rem; display:flex; gap:0.5rem; font-size:var(--wpc-font-size-body);">
                                        ...
                                        <?php if ($show_plan_buttons): ?>
                                                <tr>
                                                    <?php foreach ($plans as $idx => $plan): ?>
                                                            <td
                                                                style="padding:1rem; vertical-align:middle; border-right:<?php echo ($idx < count($plans) - 1) ? '1px solid var(--pt-border)' : 'none'; ?>;">
                            ...
                            <?php if ($show_footer_button && !empty($item['direct_link'])): ?>
                                    <div style="padding:1.25rem; text-align:center; border-top:1px solid hsl(var(--border));">
                                        <a href="<?php echo esc_url($item['direct_link']); ?>" target="_blank" rel="noopener noreferrer"
